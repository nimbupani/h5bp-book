...
app = {
  jsdoc: {
    scanner: new(require('jsdoc/src/scanner')
      .Scanner)(),
    parser: new(require('jsdoc/src/parser')
      .Parser)(),
...
function dump() {
  for (var i = 0, leni = arguments.length; i < leni; i++) {
    print(require('jsdoc/util/dumper')
      .dump(arguments[i]));
  }
}
...
function main() {
  ...
  if (env.opts.query) {
    env.opts.query = require('common/query')
      .toObject(env.opts.query);
  }
  ...
  for (var i = 0, l = env.opts._.length; i < l; i++) {
    if (/\bpackage\.json$/i.test(env.opts._[i])) {
      packageJson = require('fs')
        .readFileSync(env.opts._[i]);
  ...
  if (env.opts._.length > 0) { // are there any files to scan and parse?
    ...
    var packageDocs = new(require('jsdoc/package')
      .Package)(packageJson);
    ...
    require('jsdoc/augment')
      .addInherited(docs);
    require('jsdoc/borrow')
      .resolveBorrows(docs);
