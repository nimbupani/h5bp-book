...
less.Parser = function Parser(env) {
  ...
  function getLocation(index, input) {
    ...
    return {
      line: typeof(index) === 'number' ? (input.slice(0, index)
        .match(/\n/g) || "")
        .length : null,
      column: column
    };
  ...
  function LessError(e, env) {
    ...
    this.callLine = e.call && (getLocation(e.call, input)
      .line + 1);
      ...
      root.toCSS = (function(evaluate) {
        ...
        return function(options, variables) {
          ...
          if (options.yuicompress && less.mode === 'node') {
            return require('./cssmin')
              .compressor.cssmin(css);
    ...
    parsers: {
      ...
      primary: function() {
        ...
        while ((node = $(this.mixin.definition) || $(this.rule) || $(this.ruleset) ||
          $(this.mixin.call) || $(this.comment) || $(this.directive)) || $(/^[\s\n]+/)) {
          node && root.push(node);
        }
        return root;
      },
      ...
      mixin: {
        ...
        definition: function() {
          var name, params = [],
            match, ruleset, param, value, cond, variadic = false;
          ...
          if (match = $(/^([#.](?:[\w-]|\\(?:[a-fA-F0-9]{1,6} ?|[^a-fA-F0-9]))+)\s*\(/)) {
            ...
            do {
              if (input.charAt(i) === '.' && $(/^\.{3}/)) {
                variadic = true;
                break;
              } else if (param = $(this.entities.variable) || $(this.entities.literal) || $(this.entities.keyword)) {
                // Variable
...
(function(tree) {
  ...
  tree.functions = {
    ...
    hue: function(color) {
      return new(tree.Dimension)(Math.round(color.toHSL()
        .h));
    },
    ...
    alpha: function(color) {
      return new(tree.Dimension)(color.toHSL()
        .a);
    },
    ...
    escape: function(str) {
      return new(tree.Anonymous)(encodeURI(str.value)
        .replace(/=/g, "%3D")
        .replace(/:/g, "%3A")
        .replace(/#/g, "%23")
        .replace(/;/g, "%3B")
  ...
  tree.JavaScript.prototype = {
    eval: function(env) {
      ...
      var expression = this.expression.replace(/@\{([\w-]+)\}/g, function(_, name) {
        return tree.jsify(new(tree.Variable)('@' + name, that.index)
          .eval(env));
      });
...
(function(tree) {
  ...
  tree.Media.prototype = {
    ...
    evalNested: function(env) {
      ...
      //    b and c and d
      //    b and c and e
      this.features = new(tree.Value)(this.permute(path)
        .map(function(path) {
...
(function(tree) {
  ...
  tree.mixin.Call.prototype = {
    eval: function(env) {
      ...
      for (var i = 0; i < env.frames.length; i++) {
        if ((mixins = env.frames[i].find(this.selector))
          .length > 0) {
            ...
            if (mixins[m].match(args, env)) {
              try {
                Array.prototype.push.apply(
                  rules, mixins[m].eval(env, this.arguments, this.important)
                  .rules);
                match = true;
              } catch (e) {
                throw {
  ...
  tree.mixin.Definition.prototype = {
      ...
      for (var i = 0, val, name; i < this.params.length; i++) {
        if (name = this.params[i].name) {
          if (this.params[i].variadic && args) {
            ...
            frame.rules.unshift(new(tree.Rule)(name, new(tree.Expression)(varargs)
              .eval(env)));
    ...
    eval: function(env, args, important) {
      var frame = this.evalParams(env, args),
        context, _arguments = [],
        rules, start;
    ...
    match: function(args, env) {
      ...
      for (var i = 0; i < len; i++) {
        if (!this.params[i].name) {
          if (args[i].eval(env)
            .toCSS() != this.params[i].value.eval(env)
            .toCSS()) {
            return false;
          }
        }
  ...
  tree.Quoted.prototype = {
    ...
    eval: function(env) {
      ...
      var value = this.value.replace(/`([^`]+)`/g, function(_, exp) {
        return new(tree.JavaScript)(exp, that.index, true)
          .eval(env)
          .value;
      })
        .replace(/@\{([\w-]+)\}/g, function(_, name) {
          var v = new(tree.Variable)('@' + name, that.index)
            .eval(env);
  ...
  tree.Ruleset.prototype = {
    ...
    joinSelector: function(paths, context, selector) {
      ...
      for (var c = 0; c < context.length; c++) {
        paths.push(before.concat(context[c])
          .concat(after));
      }
    }
  };
...
(function(tree) {
  ...
  tree.Variable.prototype = {
    eval: function(env) {
      ...
      if (name.indexOf('@@') == 0) {
        name = '@' + new(tree.Variable)(name.slice(1))
          .eval(env)
          .value;
      }
